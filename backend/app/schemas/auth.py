...
# PLATFORM_CORE: Two-Factor Authentication schemas
class MFASetupRequest(BaseModel):
    """Schema for MFA setup request."""
    password: str  # Confirm identity before enabling MFA

class MFASetupResponse(BaseModel):
    """Schema for MFA setup response."""
    secret: str
    qr_code: str  # Base64 encoded QR code image
    backup_codes: List[str]  # Optional backup codes
...
class LoginWithMFARequest(BaseModel):
    """Schema for login with MFA code."""
    username: str
    password: str
    mfa_code: Optional[str] = Field(None, min_length=6, max_length=6, pattern=r'^\d{6}$')
