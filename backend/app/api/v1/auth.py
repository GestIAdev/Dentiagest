...
async def register_user(
    ...
    role=user_data.role or UserRole.receptionist,
        is_active=True
    )
...
async def login(
    ...
    # Update last login
    user.last_login_at = user.get_current_time()
...
# PLATFORM_CORE: Login endpoint with MFA support
...
async def login_with_mfa(
    ...
    """
    User login with email/username, password, and optional MFA code.
    Returns access and refresh tokens.
    """
...
#         )
#     
#     # TODO: Add is_email_verified field to User model
...
# PLATFORM_CORE: Two-Factor Authentication endpoints
...
async def verify_mfa_setup(
    ...
    # Verify TOTP code
    if not verify_totp_code(current_user.mfa_secret, verify_data.code):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid MFA code"
        )
...
async def disable_mfa(
    ...
    if current_user.is_mfa_enabled and not verify_totp_code(current_user.mfa_secret, disable_data.code):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid MFA code"
        )
...
async def get_mfa_status(
    ...
    return MFAStatusResponse(
        ...
        backup_codes_count=0  # Could count backup codes here
    )
