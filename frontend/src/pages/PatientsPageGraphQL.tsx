...
// 🔥 SHARED QUERY VARIABLES - Apollo cache key alignment
// Grid 3x3 = 9 cards per page for clean visual layout
...
const PatientsPageGraphQL: React.FC = () => {
  ...
  // 🔥 DEBUG: Log GraphQL read errors
  React.useEffect(() => {
    if (error) {
      console.error('🔥 GRAPHQL READ ERROR:', error);
      toast.error(`Error cargando pacientes: ${error.message}`);
    }
  }, [error]);
  ...
  const handleSave = () => {
    // Refetch after save to update list
    refetch();
    closeForm();
  };
